          ...
          %>  
           ...
           <div id="<%=Integer.valueOf(item.getId()) %>" class="item new col-md-4">
                <div class="featured-item">
                  <a href="single-product.jsp?id=<%=Integer.valueOf(item.getId()) %>"><div class="img-box"><img src="proimages/<%=item.getPicture()%>"></div><br><br></a><br><br>
                  <h4><a href="single-product.jsp?id=<%=Integer.valueOf(item.getId())  %>"><%=item.getName() %></a></h4>
                  
                  <h6>￥<%=item.getPrice() %> <script>
for (var i=1; i<= 76; i++)
{
document.write ("&nbsp;")
}
</script>id:<%=Integer.valueOf(item.getId())  %></h6>
                </div>
            </div>
